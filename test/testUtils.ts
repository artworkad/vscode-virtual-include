 ...
 */
export async function createTestFile(content: string, extension: string = '.py'): Promise<vscode.Uri> {
  ensureTestWorkspace();

  // Create a random filename
  const fileName = `test-${Math.random().toString(36).substring(2, 15)}${extension}`;
 ...
 */
export async function createTestDocument(content: string, extension: string = '.py'): Promise<vscode.TextEditor> {
 ...
 */
export async function waitForVirtualIncludeProcessed(editor: vscode.TextEditor, timeout: number = 5000): Promise<boolean> {
  return waitForCondition(() => {
    const text = editor.document.getText();
    console.log("Current text:", text); // Add logging to debug
    return text.includes('virtualIncludeStart') && text.includes('virtualIncludeEnd');
  }, timeout);
}
