...
import { createTestFile, deleteTestFile, ensureTestWorkspace, waitForVirtualIncludeProcessed } from '../testUtils';
...
suite('Self Include Prevention Tests', function () {
    ...
    setup(function () {
        ...
        warningStub = sinon.stub(vscode.window, 'showWarningMessage') as sinon.SinonStub;
    ...
    test('Should prevent a file from including itself', async function () {
        ...
        assert.ok(!text.includes('virtualIncludeStart'), 'Start marker should not be present');
        assert.ok(!text.includes('virtualIncludeEnd'), 'End marker should not be present');
