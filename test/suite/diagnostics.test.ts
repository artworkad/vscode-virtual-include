...
suite("Diagnostics Tests", function () {
  ...
  test("Should report diagnostics for missing files", async function () {
    ...
    assert.strictEqual(
      diagnosticCollectionStub.set.callCount,
      1,
    ...
    assert.ok(
      uri.fsPath === setUri.fsPath,
      "Diagnostics should be cleared for the correct URI",
    );
    assert.strictEqual(
      diagnostics.length,
      1,
      "One diagnostic should be created",
    );
    assert.strictEqual(
    ...
    assert.strictEqual(
      ...
      "Virtual Include",
      "Diagnostic source should be set",
    );
  ...
  test("Should not report diagnostics for existing files", async function () {
    ...
    const [setUri, diagnostics] = diagnosticCollectionStub.set.args[0];
    assert.ok(
      mainUri.fsPath === setUri.fsPath,
      "Diagnostic should be set for the correct URI",
    );
    assert.strictEqual(
      diagnostics.length,
      0,
      "No diagnostics should be created",
    );
  ...
  test("Should clear diagnostics for a document", async function () {
    ...
    assert.strictEqual(
      diagnosticCollectionStub.delete.callCount,
      1,
      "Diagnostic collection should be cleared",
    );
  ...
  test("Should properly dispose resources", function () {
    ...
    assert.strictEqual(
      diagnosticCollectionStub.dispose.callCount,
      1,
  ...
  test("Should create diagnostic with correct range", async function () {
    ...
    assert.strictEqual(
      ...
      expectedLine,
      "Diagnostic start line should match include directive",
    );
    assert.strictEqual(
      diagnostic.range.end.line,
      expectedLine,
      "Diagnostic end line should match include directive",
    );
    ...
    assert.strictEqual(
      ...
      line.text.indexOf("# virtual"),
      "Diagnostic should start at include directive",
    );
    assert.strictEqual(
      diagnostic.range.end.character,
      line.text.length,
      "Diagnostic should end at line end",
    );
