...
suite("Nested Includes Tests", () => {
  test("Nested include directives are neutralized", async () => {
    ...
    assert.ok(
      text.includes("function sourceHelper()"),
      "Included content not found",
    );

    // Verify that the nested include directive was neutralized
    assert.ok(
      text.includes("virtualInclude-nested"),
      "Nested include directive was not neutralized",
    );
    assert.ok(
      !text.includes("deepHelper()"),
      "Nested file content should not be included",
    );
  ...
  test("Changes to source files with nested includes propagate correctly", async () => {
    ...
    await vscode.workspace.fs.writeFile(
      sourceUri,
      Buffer.from(updatedSourceContent),
    );
    ...
    assert.ok(
      updatedText.includes("I am an updated source helper"),
      "Updated source content not found",
    );
    assert.ok(
      updatedText.includes("virtualInclude-nested"),
      "Nested include directive was not neutralized after update",
    );
