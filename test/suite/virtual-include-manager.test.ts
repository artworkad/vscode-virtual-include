...
suite("Virtual Include Manager Tests", function () {
  ...
  test("Should properly initialize and dispose resources", function () {
    ...
    const onDocumentOpenStub = sinon
    ...
    assert.strictEqual(
      manager["_documentIncludes"].size,
      0,
      "Document includes should be cleared",
    );
    assert.strictEqual(
      manager["_documentWatchers"].size,
      0,
      "Document watchers should be cleared",
    );
    assert.strictEqual(
      manager["_sourceToDocuments"].size,
      0,
      "Source to documents map should be cleared",
    );
