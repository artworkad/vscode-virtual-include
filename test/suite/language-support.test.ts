    ...
    testCases.forEach(({ language, extension, commentStyle }) => {
        test(`Support for ${language} comment style`, async function () {
            ...
            try {
                ...
                assert.strictEqual(processed, true, `Virtual include for ${language} was not processed`);

                // Verify the include was processed with correct comment style
                const text = editor.document.getText();
                assert.ok(text.includes(`${commentStyle} virtualIncludeStart`), 'Language-specific start marker not found');
                assert.ok(text.includes(`${commentStyle} virtualIncludeEnd`), 'Language-specific end marker not found');
