...
export class FileWatcher {
  ...
  public createWatcher(sourcePath: string): vscode.FileSystemWatcher {
    ...
    watcher.onDidDelete(uri => {
      if (uri.fsPath === sourcePath) {
        this._manager.uiHandler.showWarningMessage(`Included file was deleted: ${sourcePath}`);
      }
    });
    ...
    if (affectedDocuments.size > 0) {
      ...
      try {
        ...
        for (const docUri of affectedDocuments) {
          try {
            const textDocument = await vscode.workspace.openTextDocument(vscode.Uri.parse(docUri));
