...
export class UIHandler {
  ...
  public showAffectedFilesNotification(sourcePath: string): void {
    const affectedDocuments = this._manager.sourceToDocuments.get(sourcePath) || new Set<string>();
    const fileName = path.basename(sourcePath);
    const message = `${fileName} changed, affecting ${affectedDocuments.size} file${affectedDocuments.size !== 1 ? 's' : ''}`;
  ...
  public async showAffectedFilesQuickPick(sourcePath: string): Promise<void> {
    ...
    const items: vscode.QuickPickItem[] = Array.from(affectedDocuments).map(uri => {
    ...
    if (selected) {
      try {
        const document = await vscode.workspace.openTextDocument(vscode.Uri.parse(selected.detail!));
  ...
  public async saveAffectedFiles(sourcePath: string): Promise<void> {
    ...
    try {
      ...
      await vscode.window.withProgress({
        ...
        for (const uri of affectedDocuments) {
          try {
            ...
            const openedDoc = vscode.workspace.textDocuments.find(doc =>
      ...
      if (savedCount > 0) {
        ...
        this.showErrorMessage(`Failed to save ${errorCount} file(s). See console for details.`);
      } else {
