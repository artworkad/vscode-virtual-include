...
export class DocumentProcessor {
  ...
  public async processDocument(editor: vscode.TextEditor): Promise<void> {
    try {
      ...
      if (this._manager.documentWatchers.has(documentKey)) {
        for (const watcher of this._manager.documentWatchers.get(documentKey)!) {
          watcher.dispose();
        }
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
          ...
          if (this._isSelfInclude(document, filePath)) {
            // Log the issue
            console.warn(`Preventing self-include in file: ${document.uri.fsPath}`);
          ...
          try {
            if (fs.existsSync(resolvedPath)) {
              ...
              if (!this._manager.sourceToDocuments.has(resolvedPath)) {
                this._manager.sourceToDocuments.set(resolvedPath, new Set<string>());
              }
              this._manager.sourceToDocuments.get(resolvedPath)!.add(documentKey);

              // Check if already expanded
              const nextLineIndex = i + 1;

              if (nextLineIndex < lines.length && lines[nextLineIndex].trim() === languageSettings.startMarkerTemplate.trim()) {
                // Find the end marker
                let endLine = -1;
                ...
                if (endLine !== -1) {
                  ...
                  const currentContent = lines.slice(nextLineIndex + 1, endLine).join('\n');
                  ...
                  const indentedContent = content.split('\n')
                    .map(contentLine => contentLine.length > 0 ? indentation + contentLine : contentLine)
                    .join('\n');

                  // If content has changed, mark for update
                  if (currentContent !== indentedContent) {
                    console.log(`Content changed for include at line ${i}, will update`);
                    needsEdit = true;
                  } else {
                    console.log(`Content unchanged for include at line ${i}, no update needed`);
                  }
                } else {
                  // End marker missing, need to fix
                  console.log(`End marker missing for include at line ${i}, will fix`);
                  needsEdit = true;
                }
              } else {
                // Not expanded yet, mark for expansion
                console.log(`Include at line ${i} not yet expanded, will expand`);
                needsEdit = true;
              }

              // Set up file watcher
              const watcher = this._manager.fileWatcher.createWatcher(resolvedPath);
              watchers.push(watcher);
            } else {
              this._manager.uiHandler.showErrorMessage(`File not found: ${resolvedPath}`);
            }
          } catch (error) {
            console.error(`Error processing include at line ${i}: ${error}`);
            this._manager.uiHandler.showErrorMessage(`Error processing include: ${error}`);
          }
        }
      }
      ...
      if (needsEdit) {
        console.log(`Document ${document.uri} needs updates, applying changes...`);
   ...
   * @returns Promise<void>
   */
  public async updateDocument(editor: vscode.TextEditor, includeMap: Map<number, string>): Promise<void> {
    try {
      ...
      for (const [lineNumber, content] of [...includeMap.entries()].sort((a, b) => a[0] - b[0])) {
        ...
        const indentedContent = content.split('\n')
          ...
          .join('\n');
        const indentedStartMarker = indentation + languageSettings.startMarkerTemplate;
        ...
        if (nextLineIndex < lines.length && lines[nextLineIndex].trim() === languageSettings.startMarkerTemplate.trim()) {
          ...
          if (endLine !== -1) {
            // Replace content - use a WorkspaceEdit instead
            const workspaceEdit = new vscode.WorkspaceEdit();

            // Replace the content between markers
            const startPos = new vscode.Position(nextLineIndex + 1, 0);
            ...
            // Update line offset (we're replacing, so offset doesn't change)
          } else {
            // End marker missing, need to fix
            // Remove everything from the start marker and add new content
            const workspaceEdit = new vscode.WorkspaceEdit();

            const startPos = new vscode.Position(nextLineIndex + 1, 0);
            const endPos = new vscode.Position(lines.length, 0);

            workspaceEdit.replace(
              document.uri,
              new vscode.Range(startPos, endPos),
              `${indentedContent}\n${indentedEndMarker}\n` +
              lines.slice(nextLineIndex + 1).join('\n')
            );

            // Apply the edit
            await vscode.workspace.applyEdit(workspaceEdit);

            // Update line offset based on content
          ...
          const pos = new vscode.Position(adjustedLine + 1, 0);
      ...
      this._manager.uiHandler.showErrorMessage(`Extension error in updateDocument: ${error}`);
    } finally {
   ...
   * @returns string
   */
  private _resolveIncludePath(document: vscode.TextDocument, includePath: string): string {
   ...
   * @returns 
   */
  private _isSelfInclude(document: vscode.TextDocument, includePath: string): boolean {
    ...
    const isSelfInclude = (process.platform === 'win32') 
      ? normalizedIncludePath.toLowerCase() === normalizedDocPath.toLowerCase()
